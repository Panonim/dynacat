...
const (
	dockerContainerLabelHide        = "dynglance.hide"
	dockerContainerLabelName        = "dynglance.name"
	dockerContainerLabelURL         = "dynglance.url"
	dockerContainerLabelDescription = "dynglance.description"
	dockerContainerLabelSameTab     = "dynglance.same-tab"
	dockerContainerLabelIcon        = "dynglance.icon"
	dockerContainerLabelID          = "dynglance.id"
	dockerContainerLabelParent      = "dynglance.parent"
	dockerContainerLabelCategory    = "dynglance.category"
)
...
func (l *dockerContainerLabels) getOrDefault(label, def string) string {
	...
	// If the label is a dynglance key and the label is not found, fall back
	// through the project's previous names (dynacat, then the original
	// upstream glance) for backward compatibility with existing user configs.
	if strings.HasPrefix(label, "dynglance.") {
		suffix := strings.TrimPrefix(label, "dynglance.")

		if v, ok := (*l)["dynacat."+suffix]; ok && v != "" {
			return v
		}
		if v, ok := (*l)["glance."+suffix]; ok && v != "" {
			return v
		}
	}
...
func fetchDockerContainersFromSource(
	...
	for i := range containers {
		...
		for label, value := range overrides {
			container.Labels["dynglance."+label] = value
		}
	}
