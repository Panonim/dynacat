# Dynacat Home Assistant add-on

Dynacat ([homepage](https://dynacat.artur.zone/), a fork of
...
3. Find **Dynacat** in the store and click **Install**. The first build
...
configured the same way as upstream Dynacat/Glance: through
`dynacat.yml`. On first start the add-on creates a minimal starter file at:

```
/addon_configs/<addon_slug>/dynacat.yml
```
...
apply changes, or rely on Dynacat's built-in config file watcher, which
reloads most changes automatically without a restart.

See the full configuration reference at
<https://dynacat.artur.zone/configuration>.

### Authentication

If you add an `auth:` section to `dynacat.yml`, you need a secret key. Since
...
Dynacat's own auth disabled and rely on Ingress/HA authentication instead.
If you do want Dynacat-level auth (e.g. because you also expose port 8080
directly), generate a key with:

```
docker exec -it addon_dynacat dynacat secret:make
```
...
- `/config` (mapped to the add-on's `addon_config` share) holds `dynacat.yml`
